...
class BaseAPI {
    ...
    var timestamp: String {
        get {
            return UserDefaults.standard.string(forKey: timestampStorageKey()) ?? ""
        }

        set {
            UserDefaults.standard.set(newValue, forKey: timestampStorageKey())
        }
    }
    ...
    func parseUpdates(items _: [[String: Any?]], in _: NSManagedObjectContext) {}
    ...
    func updates(_ resource: String, _ completion: APIResultBlock?) -> URLSessionTask {
        ...
        request.method = .GET
        request.addUpdateCredentials(timestamp: timestamp)
...
//                    SyncManager.shared.insertPaginationTask(newPageTask)
    ...
    func upload(_ resource: String, _ modelID: NSManagedObjectID, _ completion: APIResultBlock?) -> URLSessionTask {
        ...
        if !timestamp.isEmpty {
            properties[Constants.key.json.timeStamp] = timestamp
        }
...
//                }
    ...
    func allChangedModels(completionBlock _: APIResultBlock?) -> [BaseAPITask] {
        return []
    }
}
